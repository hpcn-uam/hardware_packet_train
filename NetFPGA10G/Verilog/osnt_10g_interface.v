...
(
    ...
    // add for mario
    output [AXI_CALCULATIONS_DATA_WIDTH-1:0] tdata_calculations,
    output [(AXI_CALCULATIONS_DATA_WIDTH/8)-1:0] tstrb_calculations,
    output tvalid_calculations,
    output tlast_calculations,
  ...
  wire		pkt_start;
    ...
    // Master Stream ANTONIS
    wire [C_M_AXIS_DATA_WIDTH_INTERNAL - 1:0] m_axis_tdata_antonis;
    wire [((C_M_AXIS_DATA_WIDTH_INTERNAL / 8)) - 1:0] m_axis_tstrb_antonis;
    wire [C_M_AXIS_TUSER_WIDTH-1:0] m_axis_tuser_antonis; // Dummy AXI TUSER
    wire m_axis_tvalid_antonis;
    wire  m_axis_tready_antonis;
    wire m_axis_tlast_antonis;
		...
		if(C_DEFAULT_SRC_PORT == 8'h10)
			begin
				...
				chipscope_ila ILA(
					...
					.TRIG2(tvalid_calculations), // IN BUS [0:0]
					.TRIG3(tlast_calculations), // IN BUS [0:0]
					.TRIG4(tready_calculations) // IN BUS [0:0]
				);
